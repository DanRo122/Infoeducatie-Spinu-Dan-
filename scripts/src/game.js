...
Game.prototype.render = function() {
  ...
  if(!this.loaded){
    ...
    if(this.remaining_time <= 0){
      ...
      if(this.is_over){
        pauseGame();
        $('#stage').html("Nivel "+this.stage+" complet!");
        $('#pieces').html(this.num_lines*this.num_lines+" piece in "+(this.time_to_complete-this.remaining_time)+"s");
        $('#modal-success').modal();
      }else{
